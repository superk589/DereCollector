...
class APIClient {
    ...
    func call(base: String? = nil, path: String, userInfo: [String: Any], callback: ((MessagePackValue?) -> Void)?) {
        ...
        request.addValue(Config.default.gameVersion, forHTTPHeaderField: "APP_VER")
        request.addValue(Config.default.resVersion, forHTTPHeaderField: "RES_VER")
        request.addValue("127.0.0.1", forHTTPHeaderField: "IP_ADDRESS")
        request.addValue("iPad5,3", forHTTPHeaderField: "DEVICE_NAME")
        request.addValue(Config.default.unityVersion, forHTTPHeaderField: "X-Unity-Version")
    ...
    func checkResVersion(callback: @escaping (String) -> Void) {
        let args: [String: Any] = [
            "campaign_data": "",
        ...
        call(path: "/load/check", userInfo: args, callback: { msg in
            guard let resVersion = msg?[.string("data_headers")]?[.string("required_res_ver")]?.stringValue else { return }
            callback(resVersion)
        })
    }
